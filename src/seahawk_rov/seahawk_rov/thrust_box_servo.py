...
class ThrustBoxServo:
    ...
    def receive_thruster(self, message:Float32MultiArray):
        for thruster in self.thruster_map:
            print(f"Thruster {thruster} | {seahawk_rov.float_to_pwm(seahawk_rov.clamp(message.data[thruster], -1.0, 1.0))} μs")
            self.kit.servo[thruster].angle = seahawk_rov.float_to_pwm(seahawk_rov.clamp(message.data[thruster], -1.0, 1.0))
